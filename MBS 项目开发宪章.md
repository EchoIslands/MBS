...
6. **生产数据必保护**：项目一旦录入真实用户信息（顾客、预约、店铺数据），任何部署切换、环境变量修改、域名迁移、数据库变更都必须以"不影响真实用户和真实数据"为前提。操作前需明确告知风险，操作后需验证数据完整性。
7. **UI 三端一致**：网页端、公众号 H5、微信小程序面向顾客的可视元素（颜色、字体、按钮、卡片、图标、布局、状态徽章、空状态）必须统一。任何一端的界面调整，必须同步评估并改造另外两端，禁止各自为政。设计 Token、通用图标、页面结构应以 H5 顾客端为基准，小程序通过 CSS 变量与通用组件复刻。
...
| 关键词 | 导航 |
|--------|------|
| 部署报错 / Vercel 500 / API 500 | 3.7 部署与运维、3.7 坑 24.6 环境变量污染、3.14.7 Vercel Runtime 请求体解析 |
...
| 环境变量 / .env | 3.7 部署与运维、3.7 坑 24.6 环境变量污染、3.14.4 环境变量配置 |
...
| 坑 24.6 | Vercel 环境变量值被污染导致 API 500 | 严重 ⭐ | 3.7 |
...
### 坑 24.6：Vercel 环境变量值被污染导致 API 500（严重 ⭐）

- **场景**：Vercel 部署成功、构建无报错，但所有依赖 Supabase 的 API 返回 500，报错信息为 `TypeError: Headers.set: "...\n..." is an invalid header value`。
- **现象时间线**：
  1. 修复模块加载问题后，`/api/health` 正常；
  2. `/api/bookings` 等需要查询数据库的接口仍然 500；
  3. 错误响应显示 `SUPABASE_SERVICE_ROLE_KEY` 的值末尾带上了换行符和 `VITE_USE_REAL_API=true`。
- **根因**：在 Vercel 后台添加环境变量时，把两段内容一起粘贴进了同一个 Value 框。例如把：
  ```text
  <SUPABASE_SERVICE_ROLE_KEY 值>
  VITE_USE_REAL_API=true
  ```
  全部放进了 `SUPABASE_SERVICE_ROLE_KEY` 的值里。Supabase 客户端用该值设置 `Authorization` 请求头时，HTTP Header 不允许换行符，于是抛出 `Headers.set` 错误。真实 key 值不应出现在任何文档或日志中。
- **解决**：
  1. 登录 Vercel → Project Settings → Environment Variables；
  2. 编辑 `SUPABASE_SERVICE_ROLE_KEY`，只保留 key 本身，删除末尾任何换行、空格、`VITE_USE_REAL_API=true` 等不属于 key 的字符；
  3. 确认 `VITE_USE_REAL_API` 是**独立的环境变量**，Key 为 `VITE_USE_REAL_API`，Value 为 `true`；
  4. 同样检查 `SUPABASE_URL`，确保值是干净的 URL；
  5. 保存后重新部署（环境变量修改不会自动触发重部署）。
- **关键验证点**：
  - 修改并部署后，访问 `/api/bookings?shopId=shop1&dateStart=...` 应返回正常数组或空数组，不再 500；
  - Vercel Functions 日志不再出现 `Headers.set` 相关报错；
  - `/api/customers`、`/api/settlements` 等依赖 Supabase 的接口恢复正常（未登录时返回 401 属于正常）。
- **教训**：
  1. **环境变量值必须逐条复制粘贴**，禁止把 `.env` 整段内容一次性贴进一个输入框；
  2. 修改环境变量后必须**手动重新部署**，Vercel 不会自动使用新值；
  3. API 500 且 `/api/health` 正常时，优先怀疑环境变量或数据库连接问题，而不是代码逻辑；
  4. 错误信息被吞掉会大幅增加排查时间，关键路径的错误响应应暴露 `details` 供快速定位。
- **相关文件**：Vercel 项目设置、`.env`、Supabase 客户端初始化文件。
- **相关坑**：3.7 坑 21.5 API 返回 500 Internal Server Error、3.7 坑 22 环境变量未在部署平台设置、3.14.4 环境变量配置。
...
- **场景**：早期版本中，顾客是否为股东需要店铺端在“客户管理”或“客户表格管理”页面手动开启 `isStockholder` 开关，推荐奖励也没有自动计算，导致权益发放依赖人工操作。
- **根因**：
...
- **教训**：
  - 业务规则（尤其是比例、阈值）必须落到代码和宪章，不能只停留在口头；
  - 自动化流程必须考虑幂等性，防止重复执行导致资损；
  - 权益发放涉及多个表更新时，要记录变更流水，方便审计。
- **相关文件**：`api/routes/index.ts`、`schema_complete.sql`、`shared/types.ts`。

#### 2. 顾客端提现入口（坑 56）

- **场景**：顾客成为股东后获得了可提现返现余额，但 H5 个人中心和微信小程序个人中心都没有申请提现入口，权益无法变现。
- **根因**：
  - 只实现了余额展示，未设计提现申请流程；
  - 对“如何提现”没有明确产品方案，导致开发遗漏。
- **解决**：
  - 产品方案确定为两种提现方式，由顾客二选一：
    1. **抵扣消费**：申请后立即扣减 `withdrawable_referral_amount`，余额转入消费抵扣（当前实现为直接扣减并标记记录为 `paid`）；
    2. **微信提现**：提交申请后进入待审核状态（`pending`），店铺端审核通过并实际打款后标记为 `paid`。
  - H5 个人中心（`src/pages/customer/Profile.tsx`）新增“申请提现”按钮和提现弹窗；
  - 小程序个人中心（`mini-program/pages/profile/profile.js` / `.wxml` / `.scss`）复刻同款弹窗；
  - 两端共用后端 `/api/withdrawals` 接口，保持请求参数、状态文案、余额计算完全一致。
- **关键验证点**：
  - H5 和小程序都能打开提现弹窗；
  - 输入金额超过可提现余额时给出明确提示；
  - 选择“抵扣消费”后余额即时扣减；选择“微信提现”后余额冻结（记录为 `pending`，顾客侧显示为已申请）。
- **教训**：
  - 权益到账后必须同时规划变现路径，否则功能半拉子；
  - 同一功能在 H5 和小程序两端必须同步设计、同步验收，不能先只做一端。
- **相关文件**：`src/pages/customer/Profile.tsx`、`mini-program/pages/profile/profile.js`、`mini-program/pages/profile/profile.wxml`、`src/api.ts`。

#### 3. 店铺端提现审核与状态流转（坑 57）

- **场景**：顾客提交微信提现申请后，店铺端没有审核界面，无法完成“审核通过—实际打款—标记已打款”的闭环。
- **根因**：
  - 只实现了顾客端申请接口，缺少店铺端查询和处理接口；
  - 没有区分 `approved`（已通过待打款）和 `paid`（已打款）状态；
  - 微信提现的余额扣减时机不明确，容易出现“已审核但未打款却已扣款”或“重复打款”。
- **解决**：
...
- **关键验证点**：
  ...
  - 消费抵扣记录自动完成，不进入审核流程。
- **待后续完善**：
  - 微信支付“商家转账到零钱”需要商户号、API 证书和开通权限；当前阶段先完成前端界面和后端状态管理，支付能力作为前置待办；
...
- 业务规则（比例、自动触发条件、状态机）必须第一时间写入宪章，避免后续失忆或实现偏差；
...
#### 7. 补充：Vercel GitHub App 事件机制说明

- **仓库级 Webhooks 页面为空是正常的**：Vercel GitHub App 通过 **GitHub App 级事件订阅** 接收 `push` 事件，而不是在仓库里注册传统的 webhook URL。
- 重新安装/授权 Vercel GitHub App 时，会刷新整个 installation 与授权仓库的关联，所有连接同一仓库的 Vercel 项目（包括旧项目）都会恢复自动部署。
- 因此，判断 Vercel 是否收到推送，应该以 **Deployments 是否更新** 为准，而不是以 GitHub Webhooks 页面是否有记录为准。
...
**第一步：先更新任务书/计划书**
- 任何新需求（新增字段、新页面、新角色权限）必须先落到本章节中，并在 `项目状态日志.md` 中同步更新当前状态。
- 不允许口头需求直接进代码。
